...
import { getCart, increment, decrement, removeFromCart, total as cartTotal } from "@/lib/cart";
...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-[Inter] text-[#111]">
          ...
          <>
              ...
              {cart.map((item) => (
                ...
                >
                  <div className="flex items-center space-x-5">
                    ...
                    />
                    <div>
                      <h2 className="font-medium text-lg">{item.title || item.name}</h2>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
              <span className="text-xl font-semibold">
                Tổng tiền: <span className="text-[#ff6347]">{Number(total || 0).toLocaleString("vi-VN")}₫</span>
              </span>

              <div className="flex space-x-3">
                <Button variant="outline" className="border-[#ff6347] text-[#ff6347] hover:bg-[#ff6347]/10 transition-all">
                  Tiếp tục mua hàng
                </Button>
                <Button className="bg-[#ff6347] hover:bg-[#ff6347]/90 text-white shadow-md transition-all">
                  Thanh toán ngay
                </Button>
              </div>
            </div>
