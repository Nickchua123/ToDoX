...
export const listProvinces = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không lấy được tỉnh/thành", error: err.message });
  }
};
...
export const listDistricts = async (req, res) => {
  try {
    ...
    const data = await ghnGetDistricts(
    ...
    res
      .status(500)
      .json({ message: "Không lấy được quận/huyện", error: err.message });
  }
};
...
export const listWards = async (req, res) => {
  try {
    ...
    if (!districtId)
      return res.status(400).json({ message: "districtId là bắt buộc" });
    const data = await ghnGetWards(Number(districtId));
    res.json(data);
  } catch (err) {
    res
      .status(500)
      .json({ message: "Không lấy được phường/xã", error: err.message });
  }
};
...
export const getAvailableServices = async (req, res) => {
  try {
    ...
    if (!toDistrictId)
      return res.status(400).json({ message: "Thiếu toDistrictId" });
    ...
    res
      .status(500)
      .json({ message: "Không lấy được gói dịch vụ", error: err.message });
  }
};
...
export const calculateFee = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không tính được phí giao hàng", error: err.message });
  }
};
...
export const createShippingOrder = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không tạo được vận đơn", error: err.message });
  }
};
...
export const cancelShippingOrder = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không hủy được vận đơn", error: err.message });
  }
};
...
export const getOrderDetailByClientCode = async (req, res) => {
  try {
    ...
    if (!clientOrderCode)
      return res.status(400).json({ message: "clientOrderCode là bắt buộc" });
    ...
    res
      .status(500)
      .json({
        message: "Không lấy được thông tin vận đơn",
        error: err.message,
      });
  }
};
...
export const getOrderFeeDetail = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Không lấy được chi tiết phí", error: err.message });
  }
};
