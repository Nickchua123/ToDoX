...
export const listNews = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Không lấy được tin tức", error: err.message });
  }
};
...
export const getNews = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Không lấy được bài viết", error: err.message });
  }
};

export const createNews = async (req, res) => {
  try {
    const { title, summary, content, image, publishedAt, author } = req.body || {};
    if (!title) return res.status(400).json({ message: "Thiếu tiêu đề" });
    const doc = await News.create({ title, summary, content, image, publishedAt, author });
    res.status(201).json(doc);
  } catch (err) {
    res.status(500).json({ message: "Không tạo được bài viết", error: err.message });
  }
};
...
export const updateNews = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Không cập nhật được bài viết", error: err.message });
  }
};
...
export const deleteNews = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Không xóa được bài viết", error: err.message });
  }
};
